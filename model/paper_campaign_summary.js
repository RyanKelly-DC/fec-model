...
module.exports = function(sequelize, DataTypes) {
    ...
    PaperCampaignSummary.init({
        ...
        form_type: DataTypes.STRING(255),
        filer_committee_id_number: DataTypes.STRING(255),
        ...
        begin_image_number: DataTypes.STRING(255),
        end_image_number: DataTypes.STRING(255),
        receipt_date: {
            type: DataTypes.DATEONLY,
            set: function(val) {
                if (val && !(val instanceof Date) && val.toString().match(/^[0-9]{8}$/)) {
                    this.setDataValue('receipt_date', moment(val, 'YYYYMMDD').toDate());
                } else {
                    this.setDataValue('receipt_date', val);
                }
            }
        }
    }, {
